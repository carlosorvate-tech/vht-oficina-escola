...
# ---------------------------------------------------------
# CABEÇALHO DE MARCAS E PATROCÍNIO (APLICATIVO)
# ---------------------------------------------------------
...
with col_logo_right:
    st.caption("🏆 PATROCINADOR MASTER (APOIO EDUCACIONAL)")
    if os.path.exists("logo oficina republica.jpeg"):
        st.image("logo oficina republica.jpeg", use_container_width=True)
    else:
        st.success("🔧 **AUTO MECÂNICA REPÚBLICA** - Apoio Educacional Oficial")
...
# ---------------------------------------------------------
def gerar_pdf_orcamento(oficina_nome, cliente_nome, veiculo_nome, obs_os, h_basica, vht_geral, total_mo_basica, h_diag, vht_esp, total_mo_diag, valor_pecas, total_mo_geral, total_orcamento):
    buffer = io.BytesIO()
    ...
    header_data = [
        [
            Paragraph(f"<b>{oficina_nome.upper()}</b><br/><font size=9 color='#555555'>Orçamento de Mão de Obra e Peças</font>", title_style),
            Paragraph("<b>ORDEM DE SERVIÇO</b><br/><font size=8 color='#777777'>Documento Orçamentário</font>", ParagraphStyle('RightH', parent=title_style, alignment=2, fontSize=11))
        ]
    ]
    ...
    info_data = [
        [Paragraph(f"<b>Cliente:</b> {cliente_nome}", body_normal), Paragraph(f"<b>Veículo / Placa:</b> {veiculo_nome}", body_normal)],
        [Paragraph(f"<b>Observações Técnicas:</b> {obs_os}", body_normal), ""]
    ]
    ...
    table_data = [
        ["Descrição dos Serviços / Peças", "Qtd/Horas", "Valor Unit. (R$)", "Total (R$)"],
        ["Mão de Obra Básica (Mecânica Geral)", f"{h_basica:.1f} h", f"R$ {vht_geral:.2f}", f"R$ {total_mo_basica:.2f}"],
        ["Mão de Obra Especializada (Diagnóstico)", f"{h_diag:.1f} h", f"R$ {vht_esp:.2f}", f"R$ {total_mo_diag:.2f}"],
        ["Peças e Componentes Aplicados", "--", "--", f"R$ {valor_pecas:.2f}"]
    ]
    t_items = Table(table_data, colWidths=[230, 80, 105, 105])
    ...
    totals_data = [
        ["Subtotal Mão de Obra:", f"R$ {total_mo_geral:.2f}"],
        ["Subtotal Peças:", f"R$ {valor_pecas:.2f}"],
        ["TOTAL GERAL:", f"R$ {total_orcamento:.2f}"]
    ]
    t_totals = Table(totals_data, colWidths=[370, 150])
    t_totals.setStyle(TableStyle([
        ('ALIGN', (0,0), (-1,-1), 'RIGHT'),
        ('FONTNAME', (0,0), (0,-1), 'Helvetica-Bold'),
        ('FONTNAME', (0,2), (1,2), 'Helvetica-Bold'),
        ('FONTSIZE', (0,2), (1,2), 12),
        ('TEXTCOLOR', (0,2), (1,2), colors.HexColor('#1B365D')),
    ...
    footer_text = Paragraph(
        "<font size=8 color='#777777'>Plataforma Educacional de Cálculo VHT • Propriedade Intelectual INFINITUS SISTEMAS INTELIGENTES<br/>Apoio Institucional: Auto Mecânica República</font>",
...
with tab3:
    ...
    st.caption("Gere um orçamento profissional simulando a venda das horas técnicas calculadas.")
    
    col_inputs, col_preview = st.columns([1, 1.2])
    
    with col_inputs:
        oficina_nome = st.text_input("Nome da Sua Oficina", value="Oficina Exemplo / Nome do Aluno")
    ...
    pdf_bytes = gerar_pdf_orcamento(
        oficina_nome, cliente_nome, veiculo_nome, obs_os,
        h_basica_os, vht_geral, total_mo_basica,
        h_diag_os, vht_esp, total_mo_diag,
        valor_pecas, total_mo_geral, total_orcamento
    )
    
    with col_preview:
        st.markdown(f"### 📋 Resumo da O.S. — {oficina_nome}")
        
        st.info(f"**Cliente:** {cliente_nome} | **Veículo:** {veiculo_nome}")
        st.write(f"**Observações:** {obs_os}")
        
        st.markdown("---")
        df_os = pd.DataFrame({
            "Item": ["Mão de Obra Básica", "Mão de Obra Diagnóstico", "Peças / Componentes"],
            "Qtd/Horas": [f"{h_basica_os:.1f} h", f"{h_diag_os:.1f} h", "--"],
            "Valor Unitário": [f"R$ {vht_geral:.2f}", f"R$ {vht_esp:.2f}", "--"],
            "Subtotal": [f"R$ {total_mo_basica:.2f}", f"R$ {total_mo_diag:.2f}", f"R$ {valor_pecas:.2f}"]
        })
        st.table(df_os)
        
        st.metric("TOTAL DO ORÇAMENTO", f"R$ {total_orcamento:.2f}", f"Mão de obra: R$ {total_mo_geral:.2f} | Peças: R$ {valor_pecas:.2f}")
        
        st.download_button(
            label="📄 Baixar Orçamento em PDF (Pronto para Imprimir ou Enviar no WhatsApp)",
            data=pdf_bytes,
        ...
        st.caption("🔒 *Tecnologia INFINITUS SISTEMAS INTELIGENTES • Apoio Institucional: Auto Mecânica República*")
